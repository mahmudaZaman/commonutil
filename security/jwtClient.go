...
import (
	...
	"github.com/mandatorySuicide/ts-common/resterror"
...
func addProfileClaims(claims *jwt.MapClaims, user *domain.User) {
	...
	(*claims)["picture"] = accessory.firstNotNullString(user.Picture, user.FacebookAuthInfo.Picture, user.GoogleAuthInfo.Picture)
//primary_picture
...
func DecodeToken(jwtTokenRaw string, secret string) (*jwt.MapClaims, *restError.RestError) {
	...
	if err != nil {
		return nil, restError.NewUnAuthorizedError("Unauthorized", &err)
	}
	if _, ok := token.Claims.(jwt.Claims); !ok && !token.Valid {
		return nil, restError.NewUnAuthorizedError("Unauthorized", nil)
	}
	...
	return nil, restError.NewUnAuthorizedError("Invalid token", &err)
}
